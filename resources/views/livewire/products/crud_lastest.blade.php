                    ...
                    <table class="min-w-full table-auto">
                        ...
                        @foreach($products as $product)
                            <tr class="@if($loop->even) bg-gray-50 @endif border-t">
                                <!-- Image Column -->
                                <td class="px-4 py-2" x-data="{ showModal: false, modalImage: '' }">
                                    <div class="flex space-x-2">
                                        @if (!empty($product->images) && is_array($product->images) && isset($product->images[0]))
                                            <img src="{{ asset($product->images[0]) }}"
                                                 alt="Product image"
                                                 class="h-12 w-12 md:h-12 md:w-12 object-cover border border-gray-500 rounded shadow-xl cursor-pointer"
                                                 @click="modalImage = '{{ asset($product->images[0]) }}'; showModal = true;" />
                                        @endif
                                    </div>
                                    ...
                                    <div x-show="showModal" x-cloak
                                        ...
                                        <div class="relative">
                                            <img :src="modalImage" class="max-w-full max-h-full" alt="Large product image" />
                                            <button @click="showModal = false"
                                                    class="absolute top-0 right-2 m-2 text-black text-2xl font-bold">
                                                X
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <td class="px-2 py-2 text-xs md:text-md">
                                    <a href="{{ route('product.show', ['slug' => $product->slug]) }}" target="_blank" class="cursor-pointer">
                                        <span class="bg-green-500 mr-2 p-1 rounded text-white">ID:{{ $product->id }}</span>  <span class="underline">{{$product->name}}</span>
                                    </a>
                                </td>
                                <!-- Ean Column -->
                                <td class="px-2 py-2 text-xs md:text-md">
                                        ...
                                        <div class="w-full md:w-9/12 border bg-gray-100 p-4 shadow-xl rounded">
                                            <form wire:submit.prevent="updateProduct({{ $product->id }})" enctype="multipart/form-data">
                                                ...
                                                <div class="grid grid-cols-1 gap-4">
                                                    ...
                                                    <!-- EAN -->
                                                    <div>
                                                        <label class="block text-gray-700">EAN</label>
                                                        <input
                                                            class="w-full border rounded px-4 py-2"
                                                            wire:model="ean"
                                                        />
                                                        @error('products.{{ $product->id }}.ean')
                                                        <span class="text-red-500 text-xs">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                    ...
                                                    <div class="bg-white p-3 border border-black rounded mt-8 mb-4">
                                                        <label class="block text-gray-700 font-bold mb-2">Optiuni de produs</label>
                                                        <!-- Existing Variations -->
                                                        <div>
                                                            @foreach ($productVariations as $index => $variation)
                                                                <div class="mb-4 p-4 border border-gray-300 rounded bg-white">
                                                                    <!-- Dynamic Attributes: Grouped 3 per row -->
                                                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                                                                        @foreach ($allAttributes as $attribute)
                                                                            <div>
                                                                                <label class="block text-gray-700 font-bold text-xs">{{ $attribute->name }}  </label>
                                                                                <select wire:model.defer="productVariations.{{ $index }}.attributes.{{ $attribute->name }}"
                                                                                        class="w-full px-4 py-2 border rounded text-xs">
                                                                                    <option value="">Alege {{ $attribute->name }}</option>
                                                                                    @foreach ($attribute->values as $value)
                                                                                        <option value="{{ $value->id }}"
                                                                                            {{ (isset($variation['attributes'][$attribute->name]) && $variation['attributes'][$attribute->name] == $value->id) ? 'selected' : '' }}>
                                                                                            {{ $value->value }}
                                                                                        </option>
                                                                                    @endforeach
                                                                                </select>
                                                                    ...
                                                                    <div class="flex items-center mb-4">
                                                                        <div class="w-full md:w-1/2">
                                                                            <label class="block text-gray-700 font-bold text-xs">Imagine variantă</label>
                                                                            <input type="file" wire:model.defer="productVariations.{{ $index }}.image"
                                                                        ...
                                                                        <div class="w-full md:w-1/2 flex items-center justify-center">
                                                                            <img src="{{ !empty($variation['image']) ? asset($variation['image']) : asset('storage/images/placeholder_product.webp') }}"
                                                                                 class="h-12 border border-gray-400 rounded shadow-xl" />
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                                                        <div>
                                                                            <label class="block text-gray-700 font-bold text-xs">Stoc</label>
                                                                            <input type="number" wire:model.defer="productVariations.{{ $index }}.stock"
                                                                                   class="w-full px-4 py-2 border rounded text-xs" />
                                                                        </div>
                                                                        <div>
                                                                            <label class="block text-gray-700 font-bold text-xs">Preț</label>
                                                                            <input type="number" step="0.01" wire:model.defer="productVariations.{{ $index }}.price"
                                                                                   class="w-full px-4 py-2 border rounded text-xs" />
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div class="text-right">
                                                                        <x-primary-link type="button" wire:click="deleteProductVariation({{ $variation['id'] }})"
