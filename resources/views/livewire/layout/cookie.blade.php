<div>
    @if(!cookie('laravel_cookie_consent')) <!-- Check if the consent cookie is not set -->
        <div x-data="{ bannerVisible: true }" <!-- Initialize Alpine.js data -->
        ...
        class="fixed bottom-0 right-0 w-full h-auto duration-300 ease-out sm:px-5 sm:pb-5 sm:w-104 lg:w-full" x-cloak>
        <div class="flex flex-col items-center justify-between w-full h-full max-w-4xl p-6 mx-auto bg-white border-t shadow-lg lg:p-8 lg:flex-row sm:border-0 sm:rounded-xl">
            ...
            <div class="flex items-end justify-end w-full pl-3 space-x-3 lg:shrink-0 lg:w-auto">
                <button @click="bannerVisible=false; document.cookie='laravel_cookie_consent=denied; path=/;';" class="inline-flex items-center justify-center shrink-0 w-1/2 px-4 py-2 text-sm font-medium tracking-wide transition-colors duration-200 bg-white border-2 rounded-md lg:w-auto text-neutral-600 hover:text-neutral-700 border-neutral-950 focus:ring-2 focus:ring-offset-2 focus:ring-neutral-900 focus:shadow-outline focus:outline-none">
                    Deny All
                </button>
                <button @click="bannerVisible=false; document.cookie='laravel_cookie_consent=true; path=/;';" class="inline-flex items-center justify-center shrink-0 w-1/2 px-4 py-2 text-sm font-medium tracking-wide text-white transition-colors duration-200 border-2 rounded-md lg:w-auto bg-neutral-950 border-neutral-950 hover:bg-neutral-900 focus:ring-2 focus:ring-offset-2 focus:ring-neutral-900 focus:shadow-outline focus:outline-none">
                    Accept All
                </button>
            </div>
        </div>
